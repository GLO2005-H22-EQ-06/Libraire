...
for index, row in df.iterrows():
    ...
    com1 = "INSERT INTO LIVRES (ISBN, titre, langue, nbrepages, description, quantity ) " \
              "VALUES (%s, %s, %s, %s, %s, %s)"
    #com2 = "INSERT INTO auteurs (nom) values (%s)"
    data1 = (
        str(row.values[5]), row.values[1][0:249], row.values[6], str(row.values[7]),
        "Rien à dire pour le moment", random.randint(1, 250))#, datetime.strptime(row.values[10], '%m/%d/%Y'))
