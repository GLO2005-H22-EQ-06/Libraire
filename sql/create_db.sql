...
BEGIN
    ...
    declare eval int;
    select sum(E.note) into eval from evaluer E where E.ISBN = new.ISBN;
    select count(E.isbn) into nbre_eval from EVALUER E where E.ISBN = NEW.ISBN;
    update livres
    set note = eval / nbre_eval
...
begin
    ...
    select prix into p_prix_initial from livres where LIVRES.ISBN = NEW.ISBN;
    select remise into p_promo from promotions P where P.id_promotion = NEW.id_promotion;
    set NEW.prix_remise = ((100-p_promo) * p_prix_initial) / 100;
end //
delimiter ;
...
Begin
    ...
    declare compte int;
    select count(A.ISBN) into compte from APPLIQUER A where A.ISBN = p_isbn;
    if compte > 0 then
        select prix_remise into p_prix from appliquer A where A.ISBN = p_isbn;
        return p_prix;
    else
        select L.prix into p_prix from livres L where L.ISBN = p_isbn;
        return p_prix;
    end if;
end //
delimiter ;
