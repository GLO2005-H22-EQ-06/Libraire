...
delimiter / / create trigger update_book_note
after
insert
  on EVALUER for each row BEGIN declare nbre_eval int;
declare eval int;
select
  sum(E.note) into eval
from
  evaluer E
where
  E.ISBN = new.ISBN;
select
  count(E.isbn) into nbre_eval
from
  EVALUER E
where
  E.ISBN = NEW.ISBN;
...
delimiter / / create trigger set_prix_remise before
insert
  on appliquer for each row begin declare p_promo integer;
declare p_prix_initial double;
select
  prix into p_prix_initial
from
  livres
where
  LIVRES.ISBN = NEW.ISBN;
...
from
  promotions P
where
  P.id_promotion = NEW.id_promotion;
set
  NEW.prix_remise = ((100 - p_promo) * p_prix_initial) / 100;
end / / delimiter;
delimiter / / create trigger update_prix_remise
after
update
  on PROMOTIONS for each row begin declare p_promo integer;
declare p_prix_initial double;
if NEW.remise != OLD.remise then
update
  appliquer A
set
  A.prix_remise = A.prix_remise * (100 - NEW.remise) /(100 - OLD.remise)
where
  A.id_promotion = NEW.id_promotion;
end if;
end / / delimiter;
delimiter / / create function get_prix_remise(p_isbn char(10)) returns double deterministic no sql Begin declare p_prix double;
declare compte int;
select
  count(A.ISBN) into compte
from
  APPLIQUER A
where
  A.ISBN = p_isbn;
...
delimiter / / create procedure transfert_from_cart_to_bill(in p_idClient char(36)) deterministic no sql begin declare taxe_rate double;
...
select
  current_timestamp into date_facture;
select
  uuid() into new_id;
set
  taxe_rate = 1.15;
set
  shipping = 15;
select
  sum(get_prix_remise(P.isbn) * P.quantity) * taxe_rate + shipping into prix_t
from
  PANIER P
where
  P.id_client = p_idClient;
open curseur;
lecteur: loop fetch curseur into p_isbn,
  p_quantity;
if lecture_complete then leave lecteur;
end if;
insert into
  facturer
values
  (p_idClient, p_isbn, new_id, p_quantity);
end loop lecteur;
insert into
  Commandes
values
  (p_idClient, new_id, date_facture, prix_t);
close curseur;
end / / delimiter;
delimiter / / create trigger delete_books
after
insert
  on Commandes for each row begin
delete from
  panier P
where
  P.id_client = NEW.id_client;
end / / delimiter;
...
from
  ...
  JOIN (
    ...
    FROM
      evaluer
    WHERE
      id_client = '5f129c57-bc74-11ec-9233-0492266e77ce'
      and ISBN = '000100039X'
  ) as tb on associer.id_client = tb.id_client;
SELECT
  *
from
  associer natural
  join evaluer
WHERE
  id_client != 'c7e869de-bcff-11ec-b89e-0492266e77ce';
select
  L.*,
  get_prix_remise(L.isbn) as prix_remise
from
