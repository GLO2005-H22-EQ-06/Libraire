...
CREATE TABLE IF NOT EXISTS FACTURER (
  ...
  FOREIGN KEY (id_client) REFERENCES Clients (id_client) ON UPDATE CASCADE ON DELETE NO ACTION,
  FOREIGN KEY (id_facture) references Commandes (id_facture) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE PANIER (
  ...
  FOREIGN KEY (ISBN) references STOCK (ISBN) ON DELETE cascade ON UPDATE CASCADE
);
